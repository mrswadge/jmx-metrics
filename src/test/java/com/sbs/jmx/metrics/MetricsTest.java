...
public class MetricsTest {
	...
	private static class CustomObjectNameFactory implements ObjectNameFactory {
		...
		public ObjectName createName( String type, String domain, String name ) {
			try {
				// Read the "name" variable as JSON. This is so we can set additional ObjectName
				// attributes.
	...
	public void init() {
		...
		reporter = JmxReporter.forRegistry( metricRegistry ).registerWith( server.getMBeanServer() ).inDomain( server.getDomain() ).createsObjectNamesWith( new CustomObjectNameFactory() ).build();
